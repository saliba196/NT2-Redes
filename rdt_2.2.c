...
#include <unistd.h>
#include <sys/time.h>
#include <errno.h>  // Required for errno and error codes
#include "rdt.h"
...
int iscorrupted(pkt *pr)
{
    ...
    unsigned short csuml = checksum((unsigned short *)&pl, pl.h.pkt_size); // Cast para unsigned short*
...
int make_pkt(pkt *p, htype_t type, hseq_t seqnum, void *msg, int msg_len)
{
    ...
    p->h.csum = checksum((unsigned short *)p, p->h.pkt_size); // Cast para unsigned short*
...
int rdt_send_static(int sockfd, void *buf, int buf_len, struct sockaddr_in *dst)
{
    ...
    if (make_pkt(&p, PKT_DATA, snd_base, buf, buf_len) < 0)
        return ERROR;

    snd_buff[snd_base % MAX_WINDOW_SIZE] = p;
    snd_ack[snd_base % MAX_WINDOW_SIZE] = FALSE;
...
resend:
    ...
    if (nr < 0 || iscorrupted(&ack) || !has_ackseq(&ack, snd_base) )
    {
        printf("rdt_send: recvfrom(PKT_ACK) || rdt_send: ACK iscorrupted || ACK !has_ackseq\n");
        goto resend;
    }

    snd_ack[ack.h.pkt_seq % MAX_WINDOW_SIZE] = TRUE;

    while(snd_ack[snd_base % MAX_WINDOW_SIZE]) {
        snd_base++;
    }
...
int rdt_recv_static(int sockfd, void *buf, int buf_len, struct sockaddr_in *src)
{
    ...
    memset(&p, 0, sizeof(pkt)); // Inicializar toda a estrutura

    // Crio um pacote de ACK
    if (make_pkt(&ack, PKT_ACK, rcv_base, NULL, 0) < 0) // MODIFIQUEI, ANTES ERA rcv_base - 1
        return ERROR;
...
int rdt_recv_dynamic(int sockfd, void *buf, int buf_len, struct sockaddr_in *src) {
    ...
    if (make_pkt(&ack, PKT_ACK, rcv_base, NULL, 0) < 0)
        return ERROR;
...
rerecv:
    ...
    if (nr < 0) {
        // Check if the error is due to a signal interruption (e.g., Ctrl+C)
        if (errno == EINTR) {
            printf("Interrupted by signal, continuing...\n");
            goto rerecv; // Go back to waiting for a packet
        }
